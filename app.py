...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(cid, "🚀 NƠI BÓNG TỐI BẮT ĐẦU... NƠI BẢN NĂNG THỨC TỈNH 🚀")
    await context.bot.send_message(cid, "👿 KHÔNG DÀNH CHO NGƯỜI YẾU TIM - CHỈ DÀNH CHO KẺ DÁM KHÁM PHÁ 👿")
    await context.bot.send_message(cid, "👀 BƯỚC VÀO ĐÂY BẠN SẼ KHÔNG MUỐN QUAY LẠI 👀")
...
bot_app.add_handler(CallbackQueryHandler(show_albums, pattern="albums"))
bot_app.add_handler(CallbackQueryHandler(back, pattern="back"))
bot_app.add_handler(CallbackQueryHandler(open_album))
...
def set_webhook():
    bot_app.bot.set_webhook(f"{BASE_URL}/webhook")
    return "Webhook set!"
...
def upload():
    ...
    <textarea name="captions"></textarea><br>
